...
Til genkendelse af tegn vil vi bruge tre forskellige metoder: Den første bruger informationer fra en egenskabsvektor (HVAD ER DET?) mens den anden analyserer enkelte pixels i hvert tegn-billede (VEKTOREN ER, SELVFØLGELIG, OGSÅ BASERET PÅ PIXELS, SÅ DET GØR DEN JO OGSÅ). I dette afsnit vil vi beskrive disse tre metoder samt en metode til at udføre syntaks analyse (HVAD VIL VI MON MED DET?) på den tegnfølge der repræsenterer et gæt på nummerpladen.
...
Ved syntaksanalyse analyseres tegn-hitlisterne (HAVD ER DET FOR NOGEN LISTER?) fra de ovenstående metoder. Analysen er relevant da et gæt fra en af de to metoder måske kan udelukkes på baggrund af overtrædelse af syntaktiske regler for hele tegnfølgen. Metoden bruges i \cite{nijhuis} og \cite{kwas}.
...
Hvis en eller flere af disse muligheder er gældende itererer (ANDET ORD FOR ITERERES. DEN KIGGER GENNEM LISTEN?) metoden ned igennem hitlisterne indtil et lovligt valg af tegn er fundet. Dette kan illustreres med følgende eksempel: Tegnfølgen \textbf{DO 45 7B3} er retuneret fra mønstergenkendelse. I denne tegnfølge forekommer der to fejl: bogstavet \textbf{O} bruges ikke på 2. position og bogstavet \textbf{B} i 6. position burde have været et tal. På 2. position itererer metoden ned igennem hitlisten til den når et bogstav som sammen med \textbf{D} danner en lovlig bostavkombination og på 6. position itereres der indtil et tal findes.
...
ALT KUNNE VEL HAVE VÆRET IMPLEMENTERET? DET ER VEL IKKE VÆSENTLIGT?
ANDRE?
