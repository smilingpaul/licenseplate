...
\title{Automatisk lokalisering og læsning af nummerplader}
...
\paragraph{Statistik}
Brug af veje og andre faciliteter kan analyseres. Hvor mange biler passerer et givet sted i løbet af et døgn? Hvor mange af disse er pendlere der kører den ene vej om morgenen og den anden om aftenen? Hvordan bevæger trafikken sig igenem en by? Hvordan fordeler trafikken sig fra dens indfaldsveje?

\paragraph{Trafikafgift}
Både London og Stockholm har i skrivende stund systemer der registrerer køretøjer der kører ind og ud a byen. Ejerne af køretøjer der bevæger sig i den indre by, får tilsendt en bøde hvis de ikke betaler for den tid de bruger i midtbyen.

\paragraph{Patruljering}
Med kameraer monteret på patruljevogne kan politiet registrere om de passerer f.eks. efterlyste køretøjer. Demonstrationen af et sådan system kan ses i \cite{canada_demo}. 

Automatisk registrering og overvågning er et kontroversielt emne, hvor folk oftest er meget for eller meget imod. I England har man i dag et meget omfattende overvågningssytem der tillader politiet at følge et køretøjer rundt i hele landet\cite{wiki_baggrund}. Som et eksempel på den modsatte situation, fik man i Tyskland i marts 2008 rettens ord for, at automatisk registrering af nummerplader ovetræder landets love om privatlivets fred\cite{tysk_ulovlig}.

%%%%%%%%%%%%%%%%%%%%%%%
% OPBYGNING AF SYSTEM %
%%%%%%%%%%%%%%%%%%%%%%%
\section{Vores system}
I vores system har vi opdelt arbejdet med at læse lokalisere og læse nummerplader i tre trin. Som det første forsøger via at finde nummerpladen i billedet. Som trin to prøver vi at klippe de syv tegn i nummerpladen ud i hver sit billeder. Som det sidste trin forsøger vi at læse de syv tegn. Systemet returnerer nummerpladens tegn som tekst.

I dette afsnit har hver af de tre trin sit eget underafsnit. I hver af disse beskriver vi den software vi har udviklet til det aktuelle trin. Vi har i forbindelse med alle tre trin arbejdet med forskellige metoder til løsning af underopgaven. Til lokalisering af nummerplader, har vi valgt at lade flere metoder arbejde sammen, mens vi i forbindelse med separation og læsning af tegn, har arbejdet med flere metoder og valgt den  mest effektive til det endelige system.

 Figur \ref{fig:system_overblik} viser et diagram over vores system. FIND PÅ NOGET MERE HER. 

\begin{figure}[htp]
\centering
\includegraphics[width=10cm]{system/illu/overordnet_system_2.png} 
\caption{Vores system som diagram. Et billede af en bil er inddata og uddata er nummerpladen som tekst.}
...
\label{sec:illu}
I dette appendiks forsøger vi at give læseren et indtryk af den type data systemet internt arbejder med. Vi viser eksempler på områder udpeget som nummerplader af lokaliseringsdelen, tætklippede nummerplader som brugt af seperationsdelen og endelig eksempler på de områder systemet har identificeret som værende tegn i nummerpladerne.

\begin{figure}[htp]
\centering
\includegraphics[width=12cm]{appendix/illu/plates_not_rotated-1.jpg} 
\caption{Nummerplader som de ser ud når det er klippet ud men endnu ikke roteret. Selvom vi har forsøgt at tage vores billeder sådan at nummerpladerne lå vandret har det været nødvendigt at rotere pladerne for at få dem til at ligge vandret.}
...
\caption{Nummerplader som de ser ud når de er klippet ud og roteret. Det er disse billeder der er inddata når tegnene i nummerpladerne skal separeres som næste trin i processen.}
...
\subsection{Tætklippede nummerplader}
\label{sec:illu:2}
Billederne i dette afsnit viser de billeder vi arbejder på når vi antager at nummerpladerne er helt tæt klippet til.

\subsection{Separerede tegn}
\label{sec:illu:chars}
Billederne i dette afsnit viser de tegn der er klippet ud af nummerpladerne i træningssættet og derefter brugt til at genkende tegnene fra nummerpladerne i kontrolsættet. Figur \vref{fig:illu:training_chars_1} til figur \vref{fig:illu:training_chars_4} viser træningsættet og figur XX til figur XX viser de billeder der er blevet opfattet som tegn i kontrolsættet og derefter forsøgt genkendt.
...
\subsection{Falske positiver}
\label{sec:illu:fejl}
I dette afsnit viser vi eksempler på systemets fejltagelser.

\subsubsection{Områder fejlagtigt identificeret som nummerpladeområder}
Samtlige fejlidentificerede nummerpladeområder.
\label{sec:illu:fejl_plader}
FEJL FRA TESTSÆT
FEJL FRA KONTROLSÆT

\subsubsection{Områder fejlagtigt identificeret som tegn}
\label{sec:illu:fejl_tegn}
FEJL FRA TESTSÆT
FEJL FRA KONTROLSÆT
