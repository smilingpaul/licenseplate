...
\paragraph{7. Find de otte dybeste dale:}
...
\paragraph{Større frihed for tegn-højder}
...
%Ved at skalere billederne ned, kunne man forbedre hastigheden for funktionen. Vores undersøgelser viste dog at dette umiddelbart er en dårlig idé, da der går for meget information tabt til at metoderne kan separere tegnene.
...
Implementeringen af metoden til tegngenkendelse ved hjælp af egenskabsvektorer findes i filerne \textit{GetMeanVectors.m} og \textit{ReadPlateFV.m}, hvis kildekode findes i afsnit \vref{code:GetMeanVectors} og \vref{code:ReadPlateFV}. Den første funktion udarbejder egenskabsvektorerne på baggrund af tegn-træningssættet mens den anden behandler testbillederne ved brug af egenskabsvektorerne.
...
Implementationen af metoden til tegngenkendelse ved hjælp af sum-billeder findes i filerne \textit{GetSumImgs.m} og \textit{ReadPlateSUM.m}, hvis kildekode findes i afsnit \vref{code:GetSumImgs} og \vref{code:ReadPlateSUM}. Den første funktion udarbejder sum-billederne for træningssættet mens den anden behandler testbillederne ved brug af sum-billederne.
...
\paragraph{2. Udregn den samlede sum for det billede der skal genkendes for hvert tegn:} For hvert tegn der skal genkendes udregnes summen fra alle sum-billederne ved at opsummere værdierne i de pixels der er markeret i billedet med tegnet der skal genkendes.
...
%\paragraph{3. Normaliser forenings-billederne:} Hvert forenings-billede normaliseres ved at dividere alle værdier i billedet med den maksimale værdi i billedet. Dermed vil alle værdier ligge mellem $0$ og $1$. Dermed vil der ikke være forskel i værdierne i sum-billeder som er lavet ud fra mange billeder og sum-billeder der lavet ud fra få billeder.
...
%Hvis vi betragter samme eksempel: for vektoren for testbilledet der ligger nærmest \textbf{A} aflæses afstanden til vektoren efter \textbf{A), eksempelvis \textbf{D}. På samme måde aflæses afstanden for vektoren for det testbillede der ligger nærmest \textbf{B} til den næste vektor i denne liste, altså \textbf{C}. De
